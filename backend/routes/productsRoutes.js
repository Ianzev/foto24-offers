...
router.put("/updatestock", async (req, res) => {
  try {
    ...
    for (const product of products) {
      ...
      const updatedStock = productData.qty_available;
      const updatedName = productData.name || ""; // Default to empty string if name is not found
      await client.query(
        "UPDATE products SET stockmalaga = $1, name = $2 WHERE sku = $3",
        [updatedStock, updatedName, sku]
      );

      // Update the stockQmedia data
      let updatedStockQmedia = 0; // Default value
      if (
        productData.suppliers_data &&
        productData.suppliers_data.length > 0 &&
        productData.suppliers_data[0].qmedia
      ) {
        updatedStockQmedia = productData.suppliers_data[0].qmedia.qty ?? 0;
      }
      await client.query(
